...
setwd("C:/Users/97252/Desktop/תואר שני/מיומנויות במדעי הנתונים")
...
Result_Q2 <- DBI::dbGetQuery(con_chocolate, "WITH sales_rep_performance AS (
    ...
    FROM salesreps sr
    JOIN orders o ON sr.srid = o.srid
...
top_sales_rep AS (
    SELECT srid, total_sales, sales_rep_Name, year_joined, avg_sales_per_year,
        sales_gap
    FROM sales_gap
    ORDER BY sales_gap DESC -- Order by the gap to find the highest positive gap
    LIMIT 1
),
top_customer AS (
...
top_product AS (
    SELECT o.srid, o.pid, p.candy_names AS Candy_name, 
...
```{sql, connection = con_chocolate, output.var = "q2 nisayon shuv"}
